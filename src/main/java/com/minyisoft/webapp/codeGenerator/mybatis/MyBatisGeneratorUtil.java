...
public class MyBatisGeneratorUtil {
	...
	public static Map<String, Object> generate(String targetPojoName, String outputDictPath,
		...
		Assert.notNull(AnnotationUtils.findAnnotation(modelClazz, ModelKey.class),
				"指定的java类并未添加注解" + ModelKey.class.getName());
		...
		for (int j = 0; j < pojoPropertyList.size(); j++) {
			...
			createTableSql.append(columnName).append(" ").append(pojoPropertyInfo.getSqlFieldType());
			...
			if (fieldName.equalsIgnoreCase("id")) {
				...
				updateSql.append(columnName).append("=").append("#{").append(fieldName).append("},");
			}
		...
		if (option.isGenerateIbatisConfig()) {
			...
			generateIbatisConfig(option.isOracleScheme(), transferObject, outputDictPath);
		}
	...
	private static void generateIbatisConfig(boolean oracelScheme, MyBatisVMTransferObject transferObject,
			String outputDictPath) {
		try {
			...
			String templateString = VelocityEngineUtils.mergeTemplateIntoString(getVe(),
					oracelScheme ? CodeGeneratorConfig.MYBATIS_ORACLE_TEMPLATE_FILE
							: CodeGeneratorConfig.MYBATIS_MYSQL_TEMPLATE_FILE, "utf-8", model);
			String resourceOutputDictPath = StringUtils.replace(outputDictPath, "src\\main\\java\\",
					"src\\main\\resources\\");
			File file = new File(resourceOutputDictPath + CodeGeneratorConfig.MAPPING_FOLDER);
			if (!file.exists()) {
				file.mkdirs();
			}
			Files.write(templateString, new File(resourceOutputDictPath + CodeGeneratorConfig.MAPPING_FOLDER
					+ transferObject.getAlias() + "SqlMap.xml"), Charsets.UTF_8);

			// 生成过滤器java类
			templateString = VelocityEngineUtils.mergeTemplateIntoString(getVe(),
					CodeGeneratorConfig.CRITERIA_TEMPLATE_FILE, "utf-8", model);
			file = new File(outputDictPath + CodeGeneratorConfig.CRITERIA_FILE_PATH);
			if (!file.exists()) {
				file.mkdirs();
			}
			Files.write(templateString, new File(outputDictPath + CodeGeneratorConfig.CRITERIA_FILE_PATH
					+ transferObject.getAlias() + "Criteria.java"), Charsets.UTF_8);

			// 生成DAO接口
			templateString = VelocityEngineUtils.mergeTemplateIntoString(getVe(),
					CodeGeneratorConfig.DAO_INTERFACE_TEMPLATE_FILE, "utf-8", model);
			file = new File(outputDictPath + CodeGeneratorConfig.DAO_INTERFACE_FILE_PATH);
			if (!file.exists()) {
				file.mkdirs();
			}
			Files.write(templateString, new File(outputDictPath + CodeGeneratorConfig.DAO_INTERFACE_FILE_PATH
					+ transferObject.getAlias() + "Dao.java"), Charsets.UTF_8);

			// 生成业务接口
			templateString = VelocityEngineUtils.mergeTemplateIntoString(getVe(),
					CodeGeneratorConfig.SERVICE_INTERFACE_TEMPLATE_FILE, "utf-8", model);
			file = new File(outputDictPath + CodeGeneratorConfig.SERVICE_INTERFACE_FILE_PATH);
			if (!file.exists()) {
				file.mkdirs();
			}
			Files.write(templateString, new File(outputDictPath + CodeGeneratorConfig.SERVICE_INTERFACE_FILE_PATH
					+ transferObject.getAlias() + "Service.java"), Charsets.UTF_8);

			// 生成业务接口实现类
			model.put("serviceName", StringUtils.uncapitalize(transferObject.getAlias()));
			templateString = VelocityEngineUtils.mergeTemplateIntoString(getVe(),
					CodeGeneratorConfig.SERVICE_IMPL_TEMPLATE_FILE, "utf-8", model);
			file = new File(outputDictPath + CodeGeneratorConfig.SERVICE_CLASS_FILE_PATH);
			if (!file.exists()) {
				file.mkdirs();
			}
			Files.write(templateString, new File(outputDictPath + CodeGeneratorConfig.SERVICE_CLASS_FILE_PATH
					+ transferObject.getAlias() + "ServiceImpl.java"), Charsets.UTF_8);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 */
	private static void generateCRUDPermission(MyBatisVMTransferObject transferObject, String shortCut,
			String pojoChinsesName, String outputDictPath) {
		...
		try {
			...
			String targetDirectory = CodeGeneratorConfig.TARGET_PROJECT_PACKAGE_RESOURCE_FILE_ROOT
					+ StringUtils.removeStart(transferObject.getPackageName(),
							CodeGeneratorConfig.CLASS_FULL_NAME_PREFIX) + "\\"
					+ CodeGeneratorConfig.PERMISSION_FILE_PATH;
			...
			Files.write(ObjectUuidUtils.createObjectID(PermissionInfo.class) + "=" + pojoShortName + ":"
					+ PERMISSION_CREATE + "," + pojoChinsesName + "_新增," + shortCut, file, Charsets.UTF_8);
			Files.append("\r\n" + ObjectUuidUtils.createObjectID(PermissionInfo.class) + "=" + pojoShortName + ":"
					+ PERMISSION_UPDATE + "," + pojoChinsesName + "_更新," + shortCut, file, Charsets.UTF_8);
			Files.append("\r\n" + ObjectUuidUtils.createObjectID(PermissionInfo.class) + "=" + pojoShortName + ":"
					+ PERMISSION_DELETE + "," + pojoChinsesName + "_删除," + shortCut, file, Charsets.UTF_8);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
			...
			for (String propertyName : propertyNameList) {
				...
				if (sqlType == null && !isEnum && !isBoolean) {
					if (!clazz.isEnum()) {
						...
						propertyInfo.setSqlFieldName(propertyInfo.getSqlFieldName() + "id");
					}
