	...
	protected void createContents() {
			...
			public void widgetSelected(final SelectionEvent arg0) {
				...
				if (!StringUtils.isBlank(selectedFile)) {
					if (selectedFile.indexOf("src") > 0) {
						String className = CodeGeneratorConfig.CLASS_FULL_NAME_PREFIX
								+ StringUtils.substringBetween(selectedFile,
										CodeGeneratorConfig.TARGET_PROJECT_PACKAGE_JAVA_FILE_ROOT, ".java").replace(
										'\\', '.');
						...
						txtOutputFileName.setText(CodeGeneratorConfig.TARGET_PROJECT_PACKAGE_RESOURCE_FILE_ROOT
								+ StringUtils.substringBetween(selectedFile,
										CodeGeneratorConfig.TARGET_PROJECT_PACKAGE_JAVA_FILE_ROOT, "model")
								+ "security\\permission\\" + simpleName + ".permission");
						txtPojoPermissionValue.setText(simpleName + ":");
					} else {
						MessageBox messageBox = new MessageBox(shell, SWT.OK);
						messageBox.setMessage("java pojo类文件应放置在包含src目录的文件路径内，请检查");
						messageBox.open();
					}
				}
			}
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent arg0) {
				...
				if (StringUtils.isBlank(txtPojoName.getText()) || StringUtils.isBlank(txtOutputFileName.getText())
				...
				try {
					...
					StringBuffer sb = new StringBuffer("\r\n").append(ObjectUuidUtils
							.createObjectID(PermissionInfo.class));
